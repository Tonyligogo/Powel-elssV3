...
function CreateQuotation() {
  ...
  const {data:customers, isPending:customerPending, isError:customerError, isSuccess:customerSuccess} = useQuery({
  ...
  function generateQoutationCode(userProvidedNumber) {
    const currentDate = new Date();
    const currentMonth = currentDate.getMonth() + 1;
    const currentYear = currentDate.getFullYear();
  
    const quotationCode = `Qtn-${userProvidedNumber}-${currentMonth}-${currentYear}`;
    ...
    <div className="p-4 bg-white rounded-lg min-h-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4">
        <div>
          ...
          <Input id="qtnNo" readOnly defaultValue="Qtn-01-07-24" />
        </div>
        <div>
